window.throttle = <T extends (...args: any[]) => any>(func: T, limit: number): ((...args: Parameters<T>) => void) => {
...
(function () {
  ...
  themeButton.addEventListener(
    ...
    window.throttle(() => {
      ...
      const currentMode = localStorage.getItem("dark_mode") || "auto";
      const nextMode =
        modes[(modes.indexOf(currentMode) + 1) % 3];
      setTheme(nextMode);
    }, 1000),
  );
  ...
  if (window.materialTheme) {
    ...
    async function generateScheme(imageFile: HTMLImageElement) {
      ...
      if (scheme && scheme.schemes && scheme.schemes.light && scheme.schemes.dark) {
    ...
    window.generateSchemeHandler = () => {
      if (window.bannerElement?.src) {
        if (window.bannerElement.complete) {
          generateScheme(window.bannerElement);
        } else {
        ...
        if (rgba && rgba.length >= 3) {
          ...
          if (!isNaN(r) && !isNaN(g) && !isNaN(b) && 
              ...
              g >= 0 && g <= 255 && 
              b >= 0 && b <= 255) {
            const scheme = extractor.generateThemeScheme({ r, g, b });
            if (scheme && scheme.schemes && scheme.schemes.light && scheme.schemes.dark) {
              document.documentElement.style.setProperty(
                "--md-sys-color-primary-light",
                extractor.hexFromArgb(scheme.schemes.light.props.primary),
